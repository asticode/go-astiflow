...
func main() {
	...
	var pre *astiavflow.PacketRateEmulator
	for _, s := range dmx.Streams() {
		...
		// Create demuxer packet interceptor
		di, err := astiavflow.NewPacketInterceptor(astiavflow.PacketInterceptorOptions{
			Group:    g,
			OnPacket: func(p *astiav.Packet, pd astiavflow.PacketDescriptor) (dispatch bool, err error) { return true, nil },
			Stop:     &astiflow.NodeStopOptions{WhenAllParentsAreDone: true},
		})
		if err != nil {
			l.Error(fmt.Errorf("main: creating %s demuxer packet interceptor failed: %w", s.CodecParameters.MediaType(), err))
			return
		}

		// Connect demuxer to demuxer packet interceptor
		if err = dmx.Connect(di, s); err != nil {
			l.Error(fmt.Errorf("main: connecting demuxer to demuxer packet interceptor failed: %w", err))
			return
		}

		// Emulate rate
		var mi *astiavflow.PacketInterceptor
		if *emulateRate {
			// Make sure packet rate emulator exists
			if pre == nil {
				// Probe
				...
				if pre, err = astiavflow.NewPacketRateEmulator(astiavflow.PacketRateEmulatorOptions{
					Group:         g,
					Stop:          &astiflow.NodeStopOptions{WhenAllParentsAreDone: true},
					TimeReference: pb.TimeReference(),
				}); err != nil {
					l.Error(fmt.Errorf("main: creating packet rate emulator failed: %w", err))
					return
				}

				// Packet rate emulator should now be demuxer's packet rate controller
				dmx.SetPacketRateController(pre)
			}

			// Connect demuxer packet interceptor to packet rate emulator
			if err = di.Connect(pre); err != nil {
				l.Error(fmt.Errorf("main: connecting demuxer packet interceptor to packet rate emulator failed: %w", err))
				return
			}

			// Create muxer packet interceptor
			if mi, err = astiavflow.NewPacketInterceptor(astiavflow.PacketInterceptorOptions{
				Group:    g,
				OnPacket: func(p *astiav.Packet, pd astiavflow.PacketDescriptor) (dispatch bool, err error) { return true, nil },
				Stop:     &astiflow.NodeStopOptions{WhenAllParentsAreDone: true},
			}); err != nil {
				l.Error(fmt.Errorf("main: creating %s muxer packet interceptor failed: %w", s.CodecParameters.MediaType(), err))
				return
			}

			// Connect packet rate emulator to muxer packet interceptor
			if err = pre.Connect(mi, di); err != nil {
				l.Error(fmt.Errorf("main: connecting packet rate emulator to muxer packet interceptor failed: %w", err))
				return
			}
		}

		// Connect demuxer packet interceptor or muxer packet interceptor to muxer
		if mi != nil {
			if err = mi.Connect(mx); err != nil {
				l.Error(fmt.Errorf("main: connecting %s muxer packet interceptor to muxer failed: %w", s.CodecParameters.MediaType(), err))
				return
			}
		} else {
			if err = di.Connect(mx); err != nil {
				l.Error(fmt.Errorf("main: connecting %s demuxer packet interceptor to muxer failed: %w", s.CodecParameters.MediaType(), err))
				return
			}
		}
	}
