...
var (
	...
	outputFormatName = flag.String("of", "", "output format name")
...
func main() {
	...
	// Open muxer
	if err := mx.Open(astiavflow.MuxerOpenOptions{
	...
	for _, s := range dmx.Streams() {
		...
		// Connect decoder or frame rate emulator to encoder
		if fre != nil {
			if err = fre.Connect(enc, dec); err != nil {
				l.Error(fmt.Errorf("main: connecting frame rate emulator to %s encoder failed: %w", s.CodecParameters.MediaType(), err))
				return
			}
		} else {
