...
func main() {
	...
	for _, s := range dmx.Streams() {
		...
		// Connect decoder or frame rate emulator to frame interceptor
		if fre != nil {
			if err = fre.Connect(fi, dec); err != nil {
				l.Error(fmt.Errorf("main: connecting frame rate emulator to %s frame interceptor failed: %w", s.CodecParameters.MediaType(), err))
				return
			}
		} else {
