...
export class NodesFilters {
    ...
    hasNode(g: Group, n: Node_): boolean {
      ...
      if (this.stats.length > 0) filters.push(this.stats.some(name => !!n.stats.find(s => s.metadata.name === name)))
    ...
    hasStat(s: Stat): boolean {
      const filters: boolean[] = []
      if (this.stats.length > 0) filters.push(this.stats.includes(s.metadata.name))
      if (filters.length > 0) {
        if (this.mode === NodesFiltersMode.and && filters.includes(false)) return false
        else if (this.mode === NodesFiltersMode.or && !filters.includes(true)) return false
      }
