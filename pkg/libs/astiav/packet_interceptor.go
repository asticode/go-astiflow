...
var (
	_ PacketHandler = (*PacketInterceptor)(nil)
)
...
func (i *PacketInterceptor) Start(ctx context.Context, cancel context.CancelFunc, tc astikit.TaskCreator) {
	...
	tc().Do(func() { i.packetHandler.start(ctx) })
}

func (i *PacketInterceptor) onPacket_(acquirePacketFunc packetHandlerAcquirePacketFunc, p *astiav.Packet, pd PacketDescriptor, n astiflow.Noder) {
	// Callback
