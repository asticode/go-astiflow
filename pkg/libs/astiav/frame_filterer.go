...
var (
	_ FrameHandler = (*FrameFilterer)(nil)
)
...
func (ff *FrameFilterer) createFilterUnlocked(o frameFiltererCreateFilterOptions) (err error) {
	...
	f.c.Add(func() {
		...
		if f.buffersinkContext != nil {
			classers.del(f.buffersinkContext.ptr())
		}
		for _, buffersrcContext := range f.buffersrcContexts {
			classers.del(buffersrcContext.ptr())
		}
		classers.del(f.g)
	})
	...
	if f.buffersinkContext, err = f.g.NewBuffersinkFilterContext(buffersink, "out"); err != nil {
	...
	for n, alias := range ff.aliases {
		...
		switch fd.MediaType {
		case astiav.MediaTypeAudio:
			buffersrcContextParameters.SetChannelLayout(fd.ChannelLayout)
			buffersrcContextParameters.SetSampleFormat(fd.SampleFormat)
			buffersrcContextParameters.SetSampleRate(fd.SampleRate)
			buffersrcContextParameters.SetTimeBase(fd.MediaDescriptor.TimeBase)
		case astiav.MediaTypeVideo:
			buffersrcContextParameters.SetColorRange(fd.ColorRange)
			buffersrcContextParameters.SetColorSpace(fd.ColorSpace)
			buffersrcContextParameters.SetHeight(fd.Height)
			buffersrcContextParameters.SetPixelFormat(fd.PixelFormat)
			buffersrcContextParameters.SetSampleAspectRatio(fd.SampleAspectRatio)
			buffersrcContextParameters.SetTimeBase(fd.MediaDescriptor.TimeBase)
			buffersrcContextParameters.SetWidth(fd.Width)
			if fd.MediaDescriptor.FrameRate.Float64() > 0 {
				buffersrcContextParameters.SetFramerate(fd.MediaDescriptor.FrameRate)
			}
		default:
		...
		if buffersrcContext, err = f.g.NewBuffersrcFilterContext(buffersrc, "in"); err != nil {
...
type frameFiltererGrapher interface {
	...
	Configure() error
	Free()
	NewBuffersinkFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersinkContexter, error)
	NewBuffersrcFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersrcContexter, error)
...
func (g *defaultFrameFiltererGrapher) NewBuffersinkFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersinkContexter, error) {
	if f == nil {
		return nil, errors.New("empty filter")
	}
	fc, err := g.FilterGraph.NewBuffersinkFilterContext(f.ptr(), name)
	if err != nil {
		return nil, err
	}
...
func (g *defaultFrameFiltererGrapher) NewBuffersrcFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersrcContexter, error) {
	if f == nil {
		return nil, errors.New("empty filter")
	}
	fc, err := g.FilterGraph.NewBuffersrcFilterContext(f.ptr(), name)
	if err != nil {
		return nil, err
	}
...
type frameFiltererBuffersrcContexter interface {
	...
	Initialize(d *astiav.Dictionary) error
	SetParameters(bfcp *astiav.BuffersrcFilterContextParameters) error
}
