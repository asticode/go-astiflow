...
type Demuxer struct {
	c                *astikit.Closer
	cumulativeStats  *demuxerCumulativeStats
...
func NewDemuxer(o DemuxerOptions) (d *Demuxer, err error) {
	...
	// Make sure to remove packet skipper when child is removed
...
func (d *Demuxer) probe(pd time.Duration) (err error) {
	...
	for {
		...
		if skippedStart, _ := d.processPacketSideData(pkt, s); skippedStart > 0 {
			// Get duration
			sd, _ := durationToTimeBase(skippedStart, s.s.MediaContext.TimeBase)
		...
		if time.Duration(astiav.RescaleQ(pts-firstPTSs[s], s.s.MediaContext.TimeBase, NanosecondRational)) > pd {
			break
		}
	}
	...
	for s, v := range firstPTSs {
		pts := astiav.RescaleQ(v, s.s.MediaContext.TimeBase, NanosecondRational)
		if firstPTS == nil {
	...
	for s, v := range firstPTSs {
		if pts := astiav.RescaleQ(v, s.s.MediaContext.TimeBase, NanosecondRational); pts == *firstPTS {
...
func (d *Demuxer) CumulativeStats() DemuxerCumulativeStats {
	return DemuxerCumulativeStats{
		...
		OutgoingBytes:    atomic.LoadUint64(&d.pd.cumulativeStats.outgoingBytes),
		OutgoingPackets:  atomic.LoadUint64(&d.pd.cumulativeStats.outgoingPackets),
	}
}
...
func (d *Demuxer) Connect(h PacketHandler, s PacketSkipper) {
	// Store skipper
	d.pd.setSkipper(h, s)
...
func (d *Demuxer) Disconnect(h PacketHandler) {
	...
	// Skipper is removed using "child.removed" event since developer may use other ways to disconnect
	// nodes (e.g. through group)
}
...
func (d *Demuxer) readFrame(ctx context.Context) bool {
	...
	// Increment stats
	atomic.AddUint64(&d.cumulativeStats.incomingBytes, uint64(pkt.Size()))
		...
		if atomic.LoadUint32(&d.l.enabled) > 0 && errors.Is(err, astiav.ErrEof) {
			...
			if d.pb.info != nil {
				...
				for idx := range d.pb.info.FirstPTS.streams {
					...
					seekTimestamp = astiav.RescaleQ(d.pb.info.FirstPTS.Value, d.pb.info.FirstPTS.Timebase, s.s.MediaContext.TimeBase)
					break
				}
			}
...
func (d *Demuxer) loop() {
	...
	if d.l.cycleCount == 0 {
		...
		for _, s := range d.ss {
			...
			// we can do about it
			ld := s.l.cycleLastPacketDuration + time.Duration(astiav.RescaleQ(s.l.cycleLastPacketPTS-*s.l.cycleFirstPacketPTS, s.s.MediaContext.TimeBase, NanosecondRational)) - s.l.cycleFirstPacketPTSRemainder
...
func (d *Demuxer) handlePacket(ctx context.Context, pkt *astiav.Packet) {
	...
	if pkt.Dts() != astiav.NoPtsValue && pkt.Pts() != astiav.NoPtsValue {
		...
		// we can do about it
		if d.l.cycleCount == 0 {
			s.l.cycleLastPacketDuration = time.Duration(astiav.RescaleQ(pkt.Duration(), s.s.MediaContext.TimeBase, NanosecondRational))
		}
		...
		if skippedStart > 0 {
			...
			skippedStartDuration, skippedStartRemainder = durationToTimeBase(skippedStart, s.s.MediaContext.TimeBase)
		...
		if atomic.LoadUint32(&d.l.enabled) > 0 && d.l.cycleCount > 0 {
			// Get duration
			var dl int64
			dl, s.l.restampRemainder = durationToTimeBase(time.Duration(d.l.cycleCount)*d.l.cycleDuration+s.l.restampRemainder+skippedStartRemainder, s.s.MediaContext.TimeBase)
		...
		if d.er.enabled {
			// Get pkt at
			pktAt := d.er.timeReference.TimeFromTimestamp(pkt.Dts(), s.s.MediaContext.TimeBase).Add(-d.er.bufferDuration)
	...
	d.pd.dispatch(Packet{
		CodecParameters: s.s.CodecParameters,
		MediaContext:    s.s.MediaContext,
		Packet:          pkt,
	})
}
