...
type packetHandlerOnPacket func(acquirePacketFunc packetHandlerAcquirePacketFunc, p *astiav.Packet, pd PacketDescriptor, n astiflow.Noder)

type packetHandlerAcquirePacketFunc func() packetHandlerReleasePacketFunc

type packetHandlerReleasePacketFunc func()
...
func (h *packetHandler) init(o packetHandlerInitOptions) {
	...
	// We don't remove initial and previous descriptors when parent is removed anymore, since
	// we may need this info later (i.e. when encoder flushes because parent has stopped)
}
...
func (h *packetHandler) HandlePacket(p Packet) {
	...
	h.c.Do(func() {
		...
		h.ch.Add(func() {
			// Everything executed outside the main loop should be protected from the closer
			h.c.Do(func() {
				// Make sure to release packet
				ignoreReleasePacket := false
				releasePacketFunc := func() {
					h.releasePacketDescriptor(pd)
					h.pp.put(pkt)
				}
				defer func() {
					if !ignoreReleasePacket {
						releasePacketFunc()
					}
				}()
				...
				// Callback
				h.onPacket(func() packetHandlerReleasePacketFunc {
					ignoreReleasePacket = true
					return releasePacketFunc
				}, pkt, pd, p.Noder)
