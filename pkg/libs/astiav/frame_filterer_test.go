...
type mockedFrameFiltererGrapher struct {
	...
	onNewBuffersinkFilterContext func(f frameFiltererFilterer, name string) (frameFiltererBuffersinkContexter, error)
	onNewBuffersrcFilterContext  func(f frameFiltererFilterer, name string) (frameFiltererBuffersrcContexter, error)
...
func (g *mockedFrameFiltererGrapher) NewBuffersinkFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersinkContexter, error) {
	if g.onNewBuffersinkFilterContext != nil {
		return g.onNewBuffersinkFilterContext(f, name)
	}
	return nil, nil
}

func (g *mockedFrameFiltererGrapher) NewBuffersrcFilterContext(f frameFiltererFilterer, name string) (frameFiltererBuffersrcContexter, error) {
	if g.onNewBuffersrcFilterContext != nil {
		return g.onNewBuffersrcFilterContext(f, name)
	}
	return nil, nil
}
...
func (c *mockedFrameFiltererBuffersrcContexter) Initialize(d *astiav.Dictionary) error {
	c.initialized = true
	return nil
}

func (c *mockedFrameFiltererBuffersrcContexter) SetParameters(bfcp *astiav.BuffersrcFilterContextParameters) error {
	c.ch = bfcp.ChannelLayout()
	c.cr = bfcp.ColorRange()
	c.cs = bfcp.ColorSpace()
	c.fr = bfcp.Framerate()
	c.h = bfcp.Height()
	c.pf = bfcp.PixelFormat()
	c.sar = bfcp.SampleAspectRatio()
	c.sf = bfcp.SampleFormat()
	c.sr = bfcp.SampleRate()
	c.tb = bfcp.TimeBase()
	c.w = bfcp.Width()
	return nil
}
...
func TestNewFrameFilterer(t *testing.T) {
	...
	withGroup(t, func(f *astiflow.Flow, g *astiflow.Group, w *astikit.Worker) {
		...
		gs.onGrapher = func(g *mockedFrameFiltererGrapher) {
			...
			case 2:
				g.onNewBuffersinkFilterContext = func(f frameFiltererFilterer, name string) (frameFiltererBuffersinkContexter, error) {
					return &mockedFrameFiltererBuffersinkContexter{mt: mediaType}, nil
				}
				g.onNewBuffersrcFilterContext = func(f frameFiltererFilterer, name string) (frameFiltererBuffersrcContexter, error) {
...
func TestFrameFiltererOnConnect(t *testing.T) {
	...
	withGroup(t, func(f *astiflow.Flow, g *astiflow.Group, w *astikit.Worker) {
		...
		require.True(t, buffersrcContext1.initialized)
		require.True(t, buffersrcContext2.initialized)
		iosName1 := alias1
		iosName2 := alias2
		bcfds := map[*mockedFrameFiltererBuffersrcContexter]FrameDescriptor{
			buffersrcContext1: fd1,
			buffersrcContext2: fd2,
		}
		buffersrcContexts := map[astiflow.Noder]frameFiltererBuffersrcContexter{
			nr1: buffersrcContext1,
			nr2: buffersrcContext2,
		}
		if buffersrcContext1.cr == fd2.ColorRange {
			iosName1 = alias2
			iosName2 = alias1
			bcfds = map[*mockedFrameFiltererBuffersrcContexter]FrameDescriptor{
				buffersrcContext1: fd2,
				buffersrcContext2: fd1,
			}
			buffersrcContexts = map[astiflow.Noder]frameFiltererBuffersrcContexter{
		...
		for ctx, fd := range bcfds {
			require.Equal(t, fd.ColorRange, ctx.cr)
			require.Equal(t, fd.ColorSpace, ctx.cs)
			require.Equal(t, fd.MediaDescriptor.FrameRate, ctx.fr)
			require.Equal(t, fd.Height, ctx.h)
			require.Equal(t, fd.PixelFormat, ctx.pf)
			require.Equal(t, fd.SampleAspectRatio, ctx.sar)
			require.Equal(t, fd.MediaDescriptor.TimeBase, ctx.tb)
			require.Equal(t, fd.Width, ctx.w)
		}
		...
		if buffersrcContext1.ch == fd2.ChannelLayout {
			bcfds = map[*mockedFrameFiltererBuffersrcContexter]FrameDescriptor{
				buffersrcContext1: fd2,
				buffersrcContext2: fd1,
			}
		...
		for ctx, fd := range bcfds {
			require.Equal(t, fd.ChannelLayout, ctx.ch)
			require.Equal(t, fd.SampleFormat, ctx.sf)
			require.Equal(t, fd.SampleRate, ctx.sr)
			require.Equal(t, fd.MediaDescriptor.TimeBase, ctx.tb)
		}
