...
func TestPacketHandler(t *testing.T) {
	...
	// acquiring packet works properly
	withGroup(t, func(f *astiflow.Flow, g *astiflow.Group, w *astikit.Worker) {
		nr1 := mocks.NewMockedNoder()
		nr2 := mocks.NewMockedNoder()
		...
		ps := []packet{}
		var releasePacketFunc func()
		h.init(packetHandlerInitOptions{
			c: c,
			n: n,
			onPacket: func(acquirePacketFunc packetHandlerAcquirePacketFunc, i *astiav.Packet, pd PacketDescriptor, n astiflow.Noder) {
				count++
				p := packet{
					n:       n,
					pts:     i.Pts(),
				...
				case 1:
					require.Len(t, h.cpp.cpp.cps, 0)
					require.Len(t, h.pp.ps, 0)
				case 2:
					require.Len(t, h.cpp.cpp.cps, 0)
					require.Len(t, h.pp.ps, 1)
					releasePacketFunc = acquirePacketFunc()
				case 3:
					require.Len(t, h.cpp.cpp.cps, 1)
					require.Len(t, h.pp.ps, 1)
					releasePacketFunc()
					require.Len(t, h.cpp.cpp.cps, 1)
					require.Len(t, h.pp.ps, 2)
		...
		require.Equal(t, PacketHandlerCumulativeStats{
			...
			AllocatedPackets:         3,
			IncomingBytes:            uint64(3 * packetSize),
			IncomingPackets:          3,
			ProcessedBytes:           uint64(3 * packetSize),
			ProcessedPackets:         3,
		}, cs)
		requireDeltaStats(t, map[string]interface{}{
			DeltaStatNameAllocatedCodecParameters:   uint64(3),
			DeltaStatNameAllocatedPackets:           uint64(3),
			astiflow.DeltaStatNameIncomingByteRate:  float64(3 * packetSize),
			astiflow.DeltaStatNameIncomingRate:      float64(3),
			astiflow.DeltaStatNameProcessedByteRate: float64(3 * packetSize),
	...
	withGroup(t, func(f *astiflow.Flow, g *astiflow.Group, w *astikit.Worker) {
		...
		h1.init(packetHandlerInitOptions{
			c: c1,
			n: n1,
			onPacket: func(acquirePacketFunc packetHandlerAcquirePacketFunc, p *astiav.Packet, pd PacketDescriptor, n astiflow.Noder) {
				count1++
			},
		})
