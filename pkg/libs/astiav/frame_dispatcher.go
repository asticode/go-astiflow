...
type frameDispatcher struct {
	cumulativeStats *frameDispatcherCumulativeStats
	n               *astiflow.Node
}
...
func newFrameDispatcher() *frameDispatcher {
	return &frameDispatcher{cumulativeStats: &frameDispatcherCumulativeStats{}}
}
...
func (fd *frameDispatcher) dispatch(f Frame) {
	...
	// Increment stats
	atomic.AddUint64(&fd.cumulativeStats.outgoingFrames, 1)
	...
	return []astikit.DeltaStat{
		{
			...
			Valuer: astikit.NewAtomicUint64RateDeltaStat(&fd.cumulativeStats.outgoingFrames),
		},
	}
}
